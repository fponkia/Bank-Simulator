 ...
 *
 */
public class BankRecords extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		BankRecords gui = new BankRecords();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btnadd) {
			...
			if(custList.insert(vr)) {
				//inserts to the list.
				File create = new File(vr.getFirstname() + vr.getLastname() + vr.getSavingacc() + vr.getChequingacc() + ".txt");	//file that needs to get created
				if(! create.exists()) {
					try {
						create.createNewFile();	//creates the file
			...
			if(custList.delete(vr)) {
				//deletes to the list.
				File del = new File(vr.getFirstname() + vr.getLastname() + vr.getSavingacc() + vr.getChequingacc() + ".txt");	//file that needs to get deleted
				if(del.exists()) {
					del.delete();	//deletes the file
				}
				else {
					JOptionPane.showMessageDialog(null, "File was not found and was not being able to delete");	//error
				}
			}
			else
